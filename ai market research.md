...
1. **Deterministic Trigger Engine**:
...
1. **`pipeline/train.py`**: LightGBM model training using `LGBMRegressor` (objective: `poisson` or `tweedie` for zero-inflated demand, or `regression_l1`), saving model artifacts locally via `joblib`.
1. **`service/engine.py`**: Business logic calculating ROP, safety stock, and evaluating inventory status to emit structured build order payloads.
1. **`api/routes.py`**:
...
1. **Tests**: Unit tests for ROP calculation and feature lag creation with dummy market data.
