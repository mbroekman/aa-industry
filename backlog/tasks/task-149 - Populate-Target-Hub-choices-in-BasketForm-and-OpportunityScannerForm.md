...
1. De daadwerkelijke productie- en industriële hubs worden in Alliance Auth opgeslagen in het `IndustryFacility` model (waar er in de database 36 actieve faciliteiten van de corp/alliantie aanwezig zijn).
1. Zowel `Basket.target_hub` als `OpportunityScanner.target_hub` zijn `ForeignKey` velden naar `IndustryFacility`, en de `clean()` methode koppelt `target_hub_id` direct aan `IndustryFacility`.
1. Omdat `IndustryFacility` nooit geraadpleegd werd in `__init__`, bleef de dropdown leeg.
...
1. Behoud eventuele extra `KnownLocation` records indien aanwezig.
1. Zorg voor deduplicatie en alfabetische sortering.
1. Schrijf unit tests om te valideren dat de dropdown opties bevat voor zowel `BasketForm` als `OpportunityScannerForm`.
...
1. In `get_target_hub_choices` worden zowel geconfigureerde `IndustryFacility` records (gefilterd op eigenaar/alliantie van de corporaties van de gebruiker, NPC-stations en productiefaciliteiten) als `KnownLocation` records opgenomen.
1. Deduplicatie ingebouwd en alfabetische sortering op naam toegepast.
1. Zowel `BasketForm` als `OpportunityScannerForm` maken nu gebruik van deze helperfunctie, zodat de dropdown niet meer afhankelijk is van asset sync alleen.
1. Unit tests toegevoegd in `industry_reforged/tests/test_ai_forms.py` die alle scenario's dekken. Alle 6 tests geslaagd.
