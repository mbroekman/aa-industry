...
- [ ] #1 Scanner filters CorpBlueprint using corporation\_\_corporation_id, ESI blueprint sync fetches all pages, CorpBlueprint is_original returns False for BPCs (quantity == -2)
...
Updated ai_manager to query CorpBlueprint using corporation\_\_corporation_id instead of corporation_id, which fixed missing blueprints scanner logic. Updated blueprints.py to use a pagination loop for ESI get blueprints call. Fixed is_original to only trigger for BPOs.
