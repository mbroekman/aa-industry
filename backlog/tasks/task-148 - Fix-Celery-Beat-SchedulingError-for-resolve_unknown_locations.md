...
1. Verwijder of deactiveer de wees-taak `industry_reforged_resolve_unknown_locations` in `PeriodicTask` van `myauth`.
1. Verifieer dat Celery Beat zonder fouten start en draait.
...
1. Wees-PeriodicTask `industry_reforged_resolve_unknown_locations` verwijderd uit `django_celery_beat.models.PeriodicTask` in `myauth`.
1. Unit tests toegevoegd in `industry_reforged/tests/test_tasks.py` voor `resolve_unknown_locations` (zowel leeg, met records, als met argumenten) - alle 4 tests geslaagd.
1. Celery beat startup geverifieerd via `celery -A myauth beat`: gestart en draait foutloos zonder SchedulingError.
