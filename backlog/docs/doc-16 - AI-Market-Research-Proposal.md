...
1. **Hybrid Data Ingestion & Multi-Hub Aggregation**: The `sync_market_data_to_ml_service` Celery task pushes a combined dataset of:
...
1. **Auto-Retraining**: The system automatically triggers the `/retrain` endpoint in the AI container after every data push (twice a day).
1. **Opportunity Scanner Fallback Strategy**:
